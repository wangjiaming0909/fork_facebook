...
void ConnectionManager::addConnection(ManagedConnection *connection,
                                      bool timeout)
{
...
void ConnectionManager::scheduleTimeout(ManagedConnection *const connection,
...
void ConnectionManager::DrainHelper::drainConnections()
{
    ...
    {
        VLOG(2) << "Idle connections cleared: " << numCleared << ", busy conns kept: " << numKept;
    }
    else
    {
...
//一般private继承是为了继承实现，但是这两个函数在Callback中并没有实现啊(pure virtual)!
//!但是虽然他是private继承，却是public override， 因此这两个函数还是public的
//!因此这个Callback的借口还是有的, 既然继承了接口，那为什么不用public继承呢?
